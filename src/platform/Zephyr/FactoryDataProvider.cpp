...
CHIP_ERROR FactoryDataProvider<FlashFactoryData>::Init()
{
    ...
    ChipLogProgress(DeviceLayer, "mapping factory data partition");
      //const struct device *flash_device;
    const void *mem_ptr;
    spi_flash_mmap_handle_t handle;
    off_t factoryData = FIXED_PARTITION_OFFSET(factory_data);
    size_t factoryDataSize = FIXED_PARTITION_SIZE(factory_data);
    ChipLogProgress(DeviceLayer, "found factory data partition with size %d at address %ld", factoryDataSize, factoryData);

    //flash_device = DEVICE_DT_GET(DT_CHOSEN(zephyr_flash_controller));
    spi_flash_mmap(factoryData, factoryDataSize, SPI_FLASH_MMAP_DATA, &mem_ptr, &handle);

    ChipLogProgress(DeviceLayer, "parsing factory data partition with size %d at address %p", factoryDataSize, mem_ptr);
    if (!ParseFactoryData((uint8_t*) mem_ptr, static_cast<uint16_t>(factoryDataSize), &mFactoryData))
    {
    ...
    ChipLogProgress(DeviceLayer, "checking factory data partition");
